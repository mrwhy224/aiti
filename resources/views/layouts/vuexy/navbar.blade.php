                    ...
                    <li class="scrollable-container media-list">
                        @foreach (Auth::user()->notifications as $notification)
                            @isset($notification->data['action_url'])
                                <a class="d-flex" href="{{ $notification->data['action_url'].'?notification_id='.$notification->id }}">
                                    @else
                                        <a class="d-flex" href="#">
                            @endisset
                                ...
                                <div class="list-item d-flex align-items-start">
                                    <div class="me-1">
                                        @if($notification->data['type']=='USER')<div class="avatar bg-white" style="color: #0a345c"><i data-feather="message-circle" style="width: 2rem;height: 2rem"></i></div>
                                        @elseif($notification->data['type']=='SYSTEM')<div class="avatar bg-white" style="color: #0a345c;"><div class="avatar-content" style="font-size: 2rem;">🤖</div></div>@endif
                                    </div>
                                    <div class="list-item-body flex-grow-1">
                                        <p class="media-heading"><span class="fw-bolder">@if($notification->data['type']=='SYSTEM')<span class="badge badge-light-secondary">پیام سیستم</span> @endif{{ $notification->data['title'] }}</span></p>
